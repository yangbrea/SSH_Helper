...
runtime_shell_binary="$(mktemp "${TMPDIR:-/tmp}/ssh-native-runtime-shell.XXXXXX")"
trap 'kill "$server_pid" "$http_proxy_pid" "$socks_proxy_pid" "$kbdint_server_pid" 2>/dev/null || true; rm -f "$test_binary" "$gate_binary" "$runtime_handshake_binary" "$runtime_tcp_handshake_binary" "$runtime_password_auth_binary" "$runtime_private_key_auth_binary" "$runtime_password_exec_binary" "$runtime_direct_password_exec_binary" "$runtime_direct_private_key_exec_binary" "$runtime_direct_stderr_binary" "$runtime_direct_output_limit_binary" "$runtime_hostkey_mismatch_binary" "$runtime_hostkey_match_binary" "$runtime_transport_handoff_binary" "$runtime_proxy_exec_binary" "$runtime_shell_binary" "$port_file" "$server_err" "$key_file" "$http_proxy_port_file" "$http_proxy_err" "$socks_proxy_port_file" "$socks_proxy_err" "$kbdint_port_file" "$kbdint_server_err" "$kbdint_binary"' EXIT
"${CXX:-c++}" \
    -std=c++17 \
    -pthread \
    -Wall \
    ...
    "$project_dir/app/src/main/cpp/ssh/ssh_socket.cpp" \
    "$project_dir/app/src/test/cpp/ssh_runtime_shell_e2e_test.cpp" \
    -lssh2 \
    -lcrypto \
    -o "$runtime_shell_binary"
"$runtime_shell_binary" "$port"

runtime_pty_exec_binary="$(mktemp "${TMPDIR:-/tmp}/ssh-native-runtime-pty-exec.XXXXXX")"
trap 'kill "$server_pid" "$http_proxy_pid" "$socks_proxy_pid" "$kbdint_server_pid" 2>/dev/null || true; rm -f "$test_binary" "$gate_binary" "$runtime_handshake_binary" "$runtime_tcp_handshake_binary" "$runtime_password_auth_binary" "$runtime_private_key_auth_binary" "$runtime_password_exec_binary" "$runtime_direct_password_exec_binary" "$runtime_direct_private_key_exec_binary" "$runtime_direct_stderr_binary" "$runtime_direct_output_limit_binary" "$runtime_hostkey_mismatch_binary" "$runtime_hostkey_match_binary" "$runtime_transport_handoff_binary" "$runtime_proxy_exec_binary" "$runtime_shell_binary" "$runtime_pty_exec_binary" "$port_file" "$server_err" "$key_file" "$http_proxy_port_file" "$http_proxy_err" "$socks_proxy_port_file" "$socks_proxy_err" "$kbdint_port_file" "$kbdint_server_err" "$kbdint_binary"' EXIT
"${CXX:-c++}" \
    -std=c++17 \
    -pthread \
    -Wall \
    ...
    "$project_dir/app/src/main/cpp/ssh/ssh_shell_operation.cpp" \
    "$project_dir/app/src/main/cpp/ssh/ssh_socket.cpp" \
    "$project_dir/app/src/test/cpp/ssh_runtime_pty_exec_e2e_test.cpp" \
    -lssh2 \
    -lcrypto \
