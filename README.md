...
- **局域网发现** — 在当前 Wi-Fi/以太网通过 TCP、SSH Banner 与 mDNS 发现 SSH 主机，并尽力从 ARP 缓存补充 MAC/OUI 厂商信息
...
- 局域网扫描只允许私有、CGNAT 与 IPv4 Link-local 地址段，单次最多 1024 个地址和 16 个端口；扫描结果不会持久化
- SSH Banner 仅用于协议识别，不代表服务器身份可信；首次连接仍必须核对主机密钥指纹
- Android 新版本通常禁止普通应用读取 ARP 邻居表，MAC 缺失属于预期情况，不影响 TCP/mDNS 扫描
