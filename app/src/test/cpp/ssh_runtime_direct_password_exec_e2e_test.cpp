...
int main(int argc, char** argv) {
    ...
    auto result = runtime->submit(
        std::make_unique<sshnative::TcpPasswordExecOperation>(
            ...
            "true",
            5s));
    assert(result && "runtime must accept direct tcp password exec operation");
    ...
    while (sshnative::MonoClock::now() < stop) {
        ...
        std::cout << "runtime-direct-password-exec-ok\n";
        runtime->shutdown();
        return 0;
    }
    std::cerr << "runtime direct password exec timeout\n";
    runtime->shutdown();
    return 1;
}
