...
int main(int argc, char** argv) {
    ...
    auto result = runtime->submit(
        std::make_unique<sshnative::TcpPasswordExecOperation>(
            ...
            "stderr-test",
            5s));
    assert(result && "runtime must accept direct stderr exec operation");
    ...
    while (sshnative::MonoClock::now() < stop) {
        ...
        assert(event.payload.find("STDERR_BEGIN") != std::string::npos);
        assert(event.payload.find("native-stderr-ok") != std::string::npos);
        assert(event.payload.find("STDERR_END") != std::string::npos);
        std::cout << "runtime-direct-stderr-ok\n";
        runtime->shutdown();
        return 0;
    }
    std::cerr << "runtime direct stderr timeout\n";
    runtime->shutdown();
    return 1;
}
